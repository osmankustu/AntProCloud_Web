...
export async function AddPersonel(model: PersonelAddModel) {
  try {
    const res = await axiosInstance.post("/employees", model);
    return res;
  } catch (error) {
...
export async function UpdatePersonel(model: PersonelUpdateModel) {
  try {
    const res = await axiosInstance.put("/employees", model);
    return res;
  } catch (error) {
...
export async function GetListByDynamicPersonel(
  ...
  try {
    const res = await axiosInstance.post(
      "/employees/get-list/by-dynamic?PageIndex=" + pageIndex + "&PageSize=" + pageSize,
      dynamicQuery,
    );
    return res.data;
  } catch (error) {
...
export async function GetAllPersonel() {
  try {
    const res = await axiosInstance.get("/employees/get-list/for-select");
    return res.data;
  } catch (error) {
    console.error("Error fetching personel:", error);
    throw error;
...
export async function getPersonelByTeamId(id: number) {
  try {
    const res = await axiosInstance.get("/employees/" + id + "/team");
    return res.data;
  } catch (error) {
