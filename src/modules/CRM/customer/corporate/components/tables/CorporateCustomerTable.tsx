...
import CorporateCustomerAddForm from "../forms/CorporateCustomerAddForm";
import TableAddButton from "@/components/ui/button/TableAddButton";
import { usePermission } from "@/core/hooks/auth/usePermission";
import customerPermissions from "@/modules/CRM/customer/constants/customerPermissions";
  ...
  return (
    <>
          ...
          <Table>
            ...
            <AnimatePresence mode="wait">
              {isLoading ? (
                ...
                >
                  {corporateCustomers?.items.map((model, index) => (
                    <motion.tr
                      key={model.id}
                      custom={index}
                      variants={rowVariant}
                    ...
                    >
                      ...
                      <TableCell className="text-theme-sm py-3 text-gray-500 dark:text-gray-400">
                        {formatDate(model.updatedDate)}
                      </TableCell>
                      <TableCell className="text-theme-sm py-3 text-gray-500 dark:text-gray-400">
                        <Button
                          key={model.id}
                          size="sm"
                          onClick={() => router.push(`/management/customers/corporate/${model.id}`)}
                        >
                          Detay
                        </Button>
                      </TableCell>
      ...
      {corporateCustomers ? (
        <>
          <Pagination
            items={corporateCustomers.count}
            pageSize={corporateCustomers.size}
            pageSizes={[20, 50]}
            onChangeSize={size =>
              router.push(
      ...
      <Modal mode="wait" isOpen={isOpen} onClose={closeModal} className="m-4 max-w-[900px]">
        <CorporateCustomerAddForm isOpen={isOpen} onClose={closeModal} />
      </Modal>
    </>
  );
};
