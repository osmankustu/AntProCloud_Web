...
const ServicesTable = () => {
  ...
  const { customerOptions, fetchCustomerOptions } = useCustomerStore();
  ...
  const joinedServices = services?.items.map(service => {
    const customer = customerOptions?.items.find(c => c.id === service.customerId);
    return {
      ...service,
          ...
          <Table>
            ...
            <AnimatePresence mode="wait">
              {isLoading ? (
                ...
                >
                  {joinedServices?.map((service, index) => (
                    <motion.tr
                      key={service.id}
                      custom={index}
                      variants={rowVariant}
