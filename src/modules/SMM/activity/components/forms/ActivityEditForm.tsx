...
import { AddDocument } from "@/modules/SMM/document/service/DocumentService";
...
import { useServiceStore } from "@/modules/SMM/service/store/useServiceStore";
...
const ActivityEditForm = ({
    ...
    run(async () => {
      ...
      if (response.status === 200) {
        ...
        if (files && files.length > 0) {
          ...
          documentFormData.append("recordCode", service!.code);
  ...
  return (
    <>
      <div>
        <CardUpdateButton disabled={activity?.employeeId == ""} onClick={openModal} text={text} />
      </div>
                ...
                <div className="mt-3 grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-1">
                  <div>
                    ...
                    <Select
                      ...
                      onChange={(value: any) => setUpdateField("status", value?.id)}
                      getOptionLabel={e => e.label}
                      getOptionValue={e => e.id}
                      value={serviceStatusData.find(i => i.id === updateFormData.status)}
