...
import { PersonelUpdateModel } from "../../../WFM/employee/model/personel";
import { UserChangePasswordModel } from "../model/user";
...
export async function UpdateFromAuth(model: UserChangePasswordModel) {
  try {
...
export async function GetPersonelFromTenant() {
  try {
    const res = await axiosInstance.get("/employees/get-from-user");
    return res.data;
  } catch (error) {
    console.error("get user for identity service", error);
    throw error;
...
export async function UpdatePersonelFromTenant(model: PersonelUpdateModel) {
  try {
    const res = await axiosInstance.put("/employees/from-user", model);
    return res;
  } catch (error) {
    console.error("get user for identity service", error);
    throw error;
